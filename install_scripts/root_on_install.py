...
# Keep this function in sync with setuid_root_on_install.py
...
if __name__ == "__main__":
    ...
    if "MESON_INSTALL_QUIET" not in os.environ:
        base = (
            f"Making sure that the installed executable '{exe_path}' is owned by root"
        )
        if dry_run:
            base += " [dry run]..."
        else:
    ...
    errmsg = """
ERROR: Installer has insufficient permissions to 'chown root:root' the installed
       executable. Please run 'sudo meson install' instead of 'meson install'.
       If you want to manage SETUID manually, please configure your builddir
       with -Droot_on_install=false -Dset_setuid_on_install=false
    """.lstrip()

    try:
        os.chown(exe_path, 0, 0)
